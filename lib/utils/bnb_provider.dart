...
class BnbProvider {
  ...
  //   client = Web3Client(_rpcUrl, http.Client());
  // }
  // Method to validate private key
  bool validateKey(String privateKey) {
    // Example validation: Private key must be 64 characters long (hex format)
  ...
  Future<String> createWallet() async {
    ...
    final encryptedKey = base64.encode(utf8.encode(privateKeyHex));

    print('Generated Wallet Address: $walletAddress');
    print('Generated Private Key (Hex): $privateKeyHex');
    print('Encrypted Private Key (Base64): $encryptedKey');
    ...
    // Debug: Read back the saved private key
    final testPrivateKey = await storage.read(key: 'private_key');
    print('Test Read Private Key (Base64): $testPrivateKey');
...
//   }
  ...
  Future<String?> exportWallet() async {
    ...
    if (encryptedPrivateKey != null) {
      try {
        // Decode base64 to UTF-8, returning the original private key in hex format
        final decodedPrivateKey =
            utf8.decode(base64.decode(encryptedPrivateKey));
        print('Exported Private Key (Hex): $decodedPrivateKey');
